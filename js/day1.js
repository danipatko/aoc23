...
console.log(
    ...input
        ...
        .map((x) => [
            // part 1
            x.match(/(\d)/gm).map((x) => parseInt(x)),
            // part 2
            [...x.matchAll(/(?=(\d|one|two|three|four|five|six|seven|eight|nine))/gim)]
                .map((x) => x[1])
                .map((x) =>
                    x.length > 1
                        ? // prettier-ignore
                          { one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8,nine: 9, }[x]
                        : parseInt(x)
                ),
        ])
        .map((x) => x.map((y) => 10 * y[0] + y[y.length - 1]))
        .reduce(([a1, a2], [d1, d2]) => [a1 + d1, a2 + d2], [0, 0])
);
